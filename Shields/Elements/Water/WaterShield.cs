...
namespace RunesMod.Shields.Elements.Water
{
    public class WaterShield : Shield
    {
        ...
        public override void Draw(Camera camera)
        {
            ...
            Texture2D circleMask = ModAssets.Request<Texture2D>(ModAssets.ShieldMaskTextures, "ShieldCircleGrayMask").Value;
            Texture2D noise = ModAssets.Request<Texture2D>(ModAssets.NoiseTextures, "FoamNoise").Value;

            Effect shieldEffect = ModAssets.Request<Effect>(ModAssets.Effects, "Shield").Value;

            shieldEffect.Parameters["uNoise"].SetValue(noise);
            shieldEffect.Parameters["uNoiseSize"].SetValue(0.25f);
            ...
            shieldEffect.Parameters["uNoiseFactor"].SetValue(1.5f * LightFactor);
            shieldEffect.Parameters["uCircleColor"].SetValue(Color.White.ToVector3());
            shieldEffect.Parameters["uCircleFactor"].SetValue(0.5f * LightFactor);

            shieldEffect.Parameters["uColor"]?.SetValue(new Vector3(0f, 0.5f, 2f));
            shieldEffect.Parameters["uHotColor"]?.SetValue(new Vector3(0.6f, 0.8f, 1f) * 1.25f);
            shieldEffect.Parameters["uHotFactor"]?.SetValue(2f);
            ...
            spriteBatch.Draw(circleMask, Center - camera.UnscaledPosition, null, Color.White, 0f, new(256f / 2f), 0.35f, 0, 0);
            spriteBatch.End();
        }
    }
}
