...
namespace RunesMod.Graphics
{
    public class TrailDrawer
    {
        ...
        private struct TrailPoint
        {
            ...
            public TrailPoint(Vector2 position, float angle)
            {
                Position = position;
                Angle = angle;
            }
        ...
        public void PrepareStripWithProceduralPadding(Vector2[] positions, float[] rotations, StripColorFunction colorFunction, StripHalfWidthFunction widthFunction, Vector2 offsetForAllPositions = default(Vector2), bool includeBacksides = false, bool tryStoppingOddBug = true)
        {
            ...
            for (int j = 0; j < count && (!tryStoppingOddBug || !(_temporaryPositionsCache[j] == zero)); j++)
            {
                ...
                Vector2 old = _temporaryPositionsCache[Math.Clamp(j - 1, 0, count - 1)] + offsetForAllPositions;
                Vector2 next = _temporaryPositionsCache[Math.Clamp(j + 1, 0, count - 1)] + offsetForAllPositions;

                float rot = _temporaryRotationsCache[j];
                int indexOnVertexArray = j * 2;
                float progressOnStrip = (float)j / (float)(count - 1);
                AddVertex(colorFunction, widthFunction, pos, old, next, indexOnVertexArray, progressOnStrip);
                //AddVertex(colorFunction, widthFunction, pos, rot, indexOnVertexArray, progressOnStrip);
            }
        ...
        private void AddVertex(StripColorFunction colorFunction, StripHalfWidthFunction widthFunction, Vector2 current, Vector2 old, Vector2 next, int indexOnVertexArray, float progressOnStrip)
        {
            while (indexOnVertexArray + 1 >= _vertices.Length)
            {
                Array.Resize(ref _vertices, _vertices.Length * 2);
            ...
            Vector2 dirOld = (current - old).SafeNormalize(Vector2.Zero);
            Vector2 dirNext = (next - current).SafeNormalize(Vector2.Zero);

            Vector2 tangent = (dirOld + dirNext).SafeNormalize(Vector2.Zero);

            Vector2 normal = new Vector2(-tangent.Y, tangent.X);
  
            Vector2 offset = normal * width;

            _vertices[indexOnVertexArray].Position = current + offset;
            _vertices[indexOnVertexArray + 1].Position = current - offset;
            _vertices[indexOnVertexArray].TexCoord = new Vector2(progressOnStrip, 1f);
            _vertices[indexOnVertexArray + 1].TexCoord = new Vector2(progressOnStrip, 0f);
            _vertices[indexOnVertexArray].Color = color;
            _vertices[indexOnVertexArray + 1].Color = color;
        ...
        public void DrawTrail()
        {
            ...
            {
                Main.instance.GraphicsDevice.DrawUserIndexedPrimitives(0, _vertices, 0, _vertexAmountCurrentlyMaintained, _indices, 0, _indicesAmountCurrentlyMaintained / 3);
            }
        }
    }
}
